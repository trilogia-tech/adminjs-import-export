...
const formatKey = (record: BaseRecord, column: Column) => {
  ...
  return column.callback ? column.callback(record.params[column.key]) : record.params[column.key]
}

const formatConcatKeys = (record: BaseRecord, column: Column) => {
  if (!column.concat?.keys) return null
  return column.concat.keys.map((key) => record.params[key]).join(column.concat.separator)
}
...
export const formatRecords = (records: BaseRecord[], columns: Column[]) => {
  const formattedRecords = records.map((record) => {
    ...
    columns.forEach((column) => {
      ...
      if (column.concat?.keys) formattedParams[column.name] = formatConcatKeys(record, column)
